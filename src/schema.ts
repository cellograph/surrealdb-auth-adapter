...
export function generateSchema(params: GenerateSchemaParams): DBAdapterSchemaCreation {
  ...
  function mapFieldType(tableName: string, fieldName: string, type?: string): string {
    ...
    try {
      ...
      const oauthAppTable = getModelName("oauthApplication");
      const clientIdField = getFieldName({ model: "oauthConsent", field: "clientId" });
    ...
    return ({ boolean: "bool", date: "datetime" } as Record<string, string>)[type ?? ""] ?? type ?? "any";
  }
  ...
  for (const [internalModel, tableDef] of Object.entries(tables)) {
    ...
    const fields = def["fields"] as Record<string, Record<string, unknown>> | undefined;
    if (fields) {
      for (const [internalField, field] of Object.entries(fields)) {
        ...
        lines.push(`DEFINE FIELD OVERWRITE ${fieldName} ON TABLE ${tableName} TYPE ${final};`);
      }
      ...
      for (const [internalField, field] of Object.entries(fields)) {
        const shouldIndex = toIndex &&
          (Array.isArray(toIndex) ? toIndex.includes(internalField) : toIndex === internalField);
        if (shouldIndex) {
          const fieldName = getFieldName({ model: internalModel, field: internalField });
          const unique = field["unique"] ? " UNIQUE" : "";
          lines.push(`DEFINE INDEX OVERWRITE idx_${tableName}_${fieldName} ON ${tableName} COLUMNS ${fieldName}${unique};`);
        }
      }
    }
    lines.push("");
