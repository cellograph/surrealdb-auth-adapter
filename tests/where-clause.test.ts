...
describe("buildWhereClause", () => {
  ...
  test("builds eq condition", () => {
    ...
    const result = buildWhereClause(bindings, [w({ field: "email", value: "a@b.com" })], "user", ctx);
  ...
  test("builds ne condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ operator: "ne" })], "user", ctx);
    expect(result).toBe(" WHERE name != $where_0");
  });

  test("builds gt condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "count", value: 5, operator: "gt" })], "item", ctx);
    expect(result).toBe(" WHERE count > $where_0");
  });

  test("builds gte condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "count", value: 5, operator: "gte" })], "item", ctx);
    expect(result).toBe(" WHERE count >= $where_0");
  });

  test("builds lt condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "count", value: 5, operator: "lt" })], "item", ctx);
    expect(result).toBe(" WHERE count < $where_0");
  });

  test("builds lte condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "count", value: 5, operator: "lte" })], "item", ctx);
    expect(result).toBe(" WHERE count <= $where_0");
  });

  test("builds IN condition with string array", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: ["Alice", "Bob"], operator: "in" })], "user", ctx);
  ...
  test("converts id array to RecordIds for IN condition", () => {
    const bindings: Record<string, unknown> = {};
    buildWhereClause(bindings, [w({ field: "id", value: ["a", "b"], operator: "in" })], "user", ctx);
  ...
  test("skips IN condition when array is empty", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: [], operator: "in" })], "user", ctx);
    expect(result).toBe("");
  });

  test("builds NOT IN condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: ["Alice"], operator: "not_in" })], "user", ctx);
    expect(result).toBe(" WHERE name NOT IN $where_0");
  });

  test("skips NOT IN condition when array is empty", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: [], operator: "not_in" })], "user", ctx);
    expect(result).toBe("");
  });

  test("builds contains condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "bio", value: "dev", operator: "contains" })], "user", ctx);
    expect(result).toBe(" WHERE bio CONTAINS $where_0");
  });

  test("builds starts_with condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: "Al", operator: "starts_with" })], "user", ctx);
    expect(result).toBe(" WHERE string::starts_with(name, $where_0)");
  });

  test("builds ends_with condition", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "email", value: ".com", operator: "ends_with" })], "user", ctx);
    expect(result).toBe(" WHERE string::ends_with(email, $where_0)");
  });

  test("builds case-insensitive eq using string::lowercase", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "email", value: "A@B.COM", operator: "eq", mode: "insensitive" })], "user", ctx);
    expect(result).toBe(" WHERE string::lowercase(email) = string::lowercase($where_0)");
  });

  test("builds case-insensitive contains using string::lowercase", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: "ALICE", operator: "contains", mode: "insensitive" })], "user", ctx);
    expect(result).toBe(" WHERE string::lowercase(name) CONTAINS string::lowercase($where_0)");
  });

  test("builds case-insensitive starts_with", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "name", value: "AL", operator: "starts_with", mode: "insensitive" })], "user", ctx);
    expect(result).toBe(" WHERE string::starts_with(string::lowercase(name), string::lowercase($where_0))");
  });

  test("builds case-insensitive ends_with", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ field: "email", value: ".COM", operator: "ends_with", mode: "insensitive" })], "user", ctx);
    expect(result).toBe(" WHERE string::ends_with(string::lowercase(email), string::lowercase($where_0))");
  });

  test("joins two conditions with AND connector", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [
  ...
  test("converts reference field string value to RecordId", () => {
    const bindings: Record<string, unknown> = {};
    buildWhereClause(bindings, [w({ field: "userId", value: "user-abc" })], "session", ctx);
    expect(bindings["where_0"]).toBeInstanceOf(RecordId);
  ...
  test("throws on unknown operator when strictOperators: true", () => {
    expect(() =>
      buildWhereClause({}, [w({ operator: "unknown" as CleanedWhere["operator"] })], "user", ctx, { strictOperators: true })
  ...
  test("falls back to eq on unknown operator when strictOperators: false", () => {
    const bindings: Record<string, unknown> = {};
    const result = buildWhereClause(bindings, [w({ operator: "unknown" as CleanedWhere["operator"] })], "user", ctx, { strictOperators: false });
    expect(result).toBe(" WHERE name = $where_0");
  });
});
